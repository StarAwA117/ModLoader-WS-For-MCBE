...
/**
 * 从磁盘重新读取 config.json 并同步到内存中的共享配置对象
 * 原地替换（不重新赋值 config 绑定），保证所有 Mod 持有的 instance.config 引用即时生效；
 * 结构性配置（basePath/ai/mods/utils）需配合 reloadAll 重建实例才生效。
 * @returns {object} 解析后的配置对象
 * @throws {Error} 配置文件不存在或 JSON 非法时抛出
 */
export function reloadConfig() {
	...
	// 同步命令前缀（它是 Command 静态字段，不在 instance.config 里）
	Command.commandPrefix = config.commandPrefix;
	return config;
}
...
export class ServerModManager {
	...
	/**
	 * 通知所有服务端 Mod 主客户端已切换（由 t:move 等命令调用）
	 * @param {Object} newClient - 新的主客户端连接
	 */
	static onMainClientSwitch(newClient) {
		Object.entries(this.modInstances).forEach(([name, instance]) => {
			const method = this._resolveMethod(instance, "onMainClientSwitch");
			if (method) {
				try {
					method.fn.apply(method.ctx, [newClient]);
				} catch (e) {
					shared.logger.error(`Server Mod ${name}.onMainClientSwitch 执行错误`);
					shared.logger.debug(e.message);
				}
			}
		});
