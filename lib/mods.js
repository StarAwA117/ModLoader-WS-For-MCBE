...
class ModRegistry {
	...
	_loadModsConfig() {
		...
		try {
			...
			logger.warning(`mods_config.json 解析失败: ${e.message}`);
			return new Map();
		}
	}
	...
	scan() {
		...
		for (const entry of entries) {
			...
			this.mods.set(modId, {
				...
				logger.error(`Mod ${entry.name} manifest 解析失败: ${e.message}`);
			}
		}
	...
	emit(event, data, excludeMod = null) {
		...
		for (const [modName, callbacks] of eventMap) {
			...
			for (const cb of callbacks) {
				try { cb(data); } catch (e) {
					logger.error(`EventBus: ${modName}.${event} 执行错误`);
...
export class ClientModManager {
	...
	static async load() {
		...
		for (const [name, ModClass] of Object.entries(DEFAULT_CLIENT_MODS)) {
			...
			logger.info(`Client Mod ${name} (default) 已加载`);
		}
		...
		for (const entry of modRegistry.list()) {
			...
			try {
				...
				logger.info(`Client Mod ${entry.name} 已加载`);
			} catch (e) {
				logger.error(`Client Mod ${entry.name} 加载失败: ${e.message}`);
			}
		}
	}
	...
	instantiate() {
		...
		for (const [name, ModClass] of Object.entries(ClientModManager.loadedMod)) {
			try { this._instantiateMod(name, ModClass); }
				...
				logger.error(`Client Mod ${name} 实例化失败`);
				logger.debug(e.message);
			}
		}
	...
	_instantiateMod(name, ModClass) {
			...
			for (const [mn, mod] of Object.entries(ccm.modInstances)) {
				if (mod._listeners && mod._listeners[event]) {
					for (const cb of mod._listeners[event]) {
						try { cb(data); } catch (e) { logger.error(`EventBus: ${mn}.${event} 执行错误`); }
					}
				}
			}
		};
		...
		if (startMethod) {
			try { startMethod.fn.apply(startMethod.ctx); }
				...
				logger.error(`Client Mod ${name}.start 执行错误`);
	...
	_collectCommands() {
		...
		for (const [name, instance] of Object.entries(this.modInstances)) {
			...
			try {
				...
				logger.error(`Client Mod ${name} 命令收集失败`);
				logger.debug(e.message);
			}
		}
	}
	...
	async reload(name) {
		...
		try {
			...
			this._collectCommands();
			logger.info(`Client Mod ${name} 已重载`);
			return { success: true, message: `Client Mod ${name} 已重载` };
		} catch (e) {
			logger.error(`Client Mod ${name} 重载失败: ${e.message}`);
		...
		for (const [name, instance] of Object.entries(this.modInstances)) {
			...
			if (method) {
				try { method.fn.apply(method.ctx, args); }
					...
					logger.error(`Client Mod ${name}.${methodName} 执行错误`);
					logger.debug(e.message);
				}
			}
		}
	...
	static async load() {
		...
		for (const entry of modRegistry.list()) {
			...
			try {
				...
				logger.info(`Server Mod ${entry.name} 已加载`);
			} catch (e) {
				logger.error(`Server Mod ${entry.name} 加载失败: ${e.message}`);
			}
		}
		// 创建单例并实例化
			...
			try {
				...
				if (startMethod) {
					...
					try { startMethod.fn.apply(startMethod.ctx); }
						...
						logger.error(`Server Mod ${name}.${startName} 执行错误`);
						logger.debug(e.stack || e.message);
					}
				}
				...
				logger.error(`Server Mod ${name} 实例化失败`);
				logger.debug(e.message);
			}
		}
	}
		...
		try {
			...
			if (typeof instance.onStart === "function") {
				try { instance.onStart(); }
					...
					logger.error(`Server Mod ${name}.onStart 执行错误`);
					logger.debug(e.message);
				}
			}
			logger.info(`Server Mod ${name} 已重载`);
			return { success: true, message: `Server Mod ${name} 已重载` };
		} catch (e) {
			logger.error(`Server Mod ${name} 重载失败: ${e.message}`);
		...
		for (const [name, instance] of Object.entries(this.modInstances)) {
			...
			if (method) {
				try { method.fn.apply(method.ctx, args); }
					...
					logger.error(`Server Mod ${name}.${methodName} 执行错误`);
					logger.debug(e.message);
				}
			}
		}
	...
	static destroy() {
		...
		for (const [name, dm] of Object.entries(inst._destroyMethods || {})) {
			try { dm.fn.apply(dm.ctx); }
				...
				logger.error(`Server Mod ${name}.onDestroy 执行错误`);
				logger.debug(e.message);
			}
		}
