...
export class ClientModManager {
	 ...
	 */
	static async load() {
		for (const [name, modPath] of Object.entries(mods.client)) {
			try {
	...
	async reload(name) {
		...
		const modPath = mods.client[name];
		if (!ModClass || !modPath) {
...
export class ServerModManager {
	 ...
	 */
	static async load() {
		for (const [name, modPath] of Object.entries(mods.server)) {
			try {
				const modModule = await import(modPath);
				const modClass = modModule.default;
				this.loadedMod[name] = modClass;
	...
	static getModPath(modName) {
		return mods.server[modName] || null;
	}
	...
	static async reload(modName) {
		// 检查 Mod 是否存在
		if (!mods.server[modName]) {
			return { success: false, message: `Mod "${modName}" 未在配置中定义` };
		}

		const modPath = mods.server[modName];
	...
	static async reloadAll() {
		...
		for (const modName of Object.keys(mods.server)) {
