...
export default class SAPIBridge {
	...
	static async detect(client) {
		...
		try {
			...
			logger.debug(`SAPI 检测失败: ${e.message}`);
			return false;
		}
	}
	...
	static async getMessages(client) {
		...
		try {
			...
			logger.debug(`SAPI getMessages 失败: ${e.message}`);
			return [];
		}
	}
	...
	static async sendMessage(client, mod, type, data = {}) {
		...
		if (Buffer.byteLength(message, "utf8") > 400) {
			logger.warning(`SAPI 消息过长: ${Buffer.byteLength(message, "utf8")} bytes`);
			return false;
		}
		...
		if (Buffer.byteLength(command, "utf8") > 461) {
			logger.warning(`SAPI 命令过长: ${Buffer.byteLength(command, "utf8")} bytes`);
			return false;
		}
		...
		try {
			...
			// statusCode 为 0 表示成功
			if (statusCode === 0) {
				logger.debug(`SAPI 发送成功: ${mod}/${type}`);
				return true;
			}

			logger.debug(`SAPI 发送失败: statusCode=${statusCode}`);
			return false;
		} catch (e) {
			logger.debug(`SAPI sendMessage 失败: ${e.message}`);
			return false;
		}
	}
}
...
export class SAPIMessageHandler {
	...
	async _poll() {
		...
		try {
			...
			if (this.commandExists === null) {
				...
				logger.info("SAPI 命令已检测到，桥接功能已启用");
			}
			...
			logger.debug(`SAPI 轮询错误: ${e.message}`);
		}
	...
	_callHandler(handler, msg) {
		...
		try {
			...
			logger.error(`SAPI 消息处理错误: ${msg.type}`);
	...
	_disable() {
		...
		this.stop();
		logger.info("SAPI 命令不存在，已禁用桥接功能");
		this._scheduleRetry();
	}
	...
	_enable() {
		...
		this.commandExists = true;
		logger.info("SAPI 命令已恢复，桥接功能已启用");
		this.start();
	}
