...
import { sapiConfig } from "../config.js";
 ...
 *
 * 命令说明（命令名见 config.js 的 sapiConfig，SAPI 端需要注册）：
...
export default class SAPIBridge {
	...
	static async detect(client) {
		...
		try {
			const data = await client.runCommand(sapiConfig.gmsg);
	...
	static async sendMessage(client, mod, type, data = {}) {
		...
		const command = `${sapiConfig.smsg} "${escaped}"`;
