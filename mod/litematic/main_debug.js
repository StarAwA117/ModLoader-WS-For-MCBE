...
export default class Litematic {
	...
	onCommand() {
		...
		return {
			op: [
				...
				Command.create("ld:help", "查看命令用法")
					...
					.setFunc((sender, name) => {
						...
						if (name) {
							...
							if (!cm) {
								this.client.tell(`§cLitematicDebug | §fError > §i没有找到命令: ${name}（输入 !ld:help 查看全部命令）`, sender);
								return;
							}
							const params = cm.parameters.length
								...
								: " §7无参数";
							this.client.tell(
								`§eLitematicDebug | §fHelp > §b${cm.name} §7用法:\n` +
								`§f说明: §7${cm.description}\n` +
								`§f参数:\n${params}`, sender
							);
						} else {
							const lines = all.map(cm => `§a!ld:${cm.name.replace(/^ld:/, "")} §7- §f${cm.description}`).join("\n");
							this.client.tell(
								`§eLitematicDebug | §fHelp > §7可用命令\n${lines}\n` +
								`§7输入 §a!ld:help <命令名> §7查看详细用法`, sender
							);
						}
					}),

				// l:create <文件> [X] [Y] [Z] [trim|raw] — 导入建筑投影
				Command.create("ld:create", "导入 Litematic 建筑投影")
					.addString("文件名", false)
					.addOptionalString("X")
					.addOptionalString("Y")
					.addOptionalString("Z")
					...
					.setFunc(async (sender, fileName, x, y, z, mode) => {
						if (this.job) {
							this.client.tell("§cLitematicDebug | §fError > §i已有导入进程运行中，请等待完成或 !ld:n 中断", sender);
							return;
						}
				...
				Command.create("ld:preview", "粒子边框 + 实体标记预览建筑位置")
					.addString("文件名", false)
					.addOptionalString("X")
					.addOptionalString("Y")
					.addOptionalString("Z")
				...
				// l:unpreview — 清除预览
				Command.create("ld:unpreview", "清除建筑预览")
					.setFunc((sender) => {
						this.clearPreview(sender);
					}),

				// l:export <文件> [导出名] [trim|raw] — 导出 .mcstructure 结构文件
				Command.create("ld:export", "导出为 MCBE 结构方块文件 (.mcstructure)")
					.addString("文件名", false)
				...
				// l:list [页码] — 浏览建筑文件
				Command.create("ld:list", "查看建筑文件列表")
				...
				Command.create("ld:id", "查看所有任务 ID")
					.setFunc((sender) => {
						this.listTasks(sender);
					}),

				// l:search <关键词> [页码] — 搜索建筑文件
				Command.create("ld:search", "搜索建筑文件")
					.addString("关键词", false)
					.addOptionalString("页码")
				...
				// l:y — 确认待执行的导入
				Command.create("ld:y", "确认导入操作")
					.setFunc(async (sender) => {
						if (!this.pending) {
							this.client.tell("§cLitematicDebug | §fError > §i没有待确认的导入任务", sender);
							return;
						}
						this.client.tell("§eLitematicDebug | §fImport > §i已确认，开始导入…", sender);
						try {
							await this.run();
						} catch (e) {
							this.client.tell(`§cLitematicDebug | §fError > §i导入出错: ${e.message}`, sender);
							this.job = null;
						}
					}),

				// l:n — 取消待确认任务或中断正在进行的导入/检查/修复
				Command.create("ld:n", "取消/中断操作")
					.setFunc((sender) => {
						if (this.job) {
							this.job.cancelled = true;
							this.client.tell("§cLitematicDebug | §fCancel > §i正在中断导入…", sender);
						} else if (this.verifyJob) {
							this.verifyJob.cancelled = true;
							this.client.tell("§cLitematicDebug | §fCancel > §i正在中断世界检查…", sender);
						} else if (this.fixJob) {
							this.fixJob.cancelled = true;
							this.client.tell("§cLitematicDebug | §fCancel > §i正在中断修复…", sender);
						} else if (this.pending) {
							this.pending = null;
							this.client.tell("§cLitematicDebug | §fCancel > §i已取消导入", sender);
						} else {
							this.client.tell("§cLitematicDebug | §fError > §i没有进行中的操作", sender);
						}
					}),

				// l:author — 作者信息
				Command.create("ld:author", "查看作者信息")
					.setFunc((sender) => {
						this.client.tell("§eLitematicDebug | §fAuthor > §iStarAwA117 & Hydrooxygen", sender);
					}),

				// l:status — 查看所有进行中任务进度（导入 / 世界检查 / 修复）
				Command.create("ld:status", "查看导入/检查/修复进度")
					.setFunc((sender) => {
						const lines = [];
						if (this.job) {
							const j = this.job;
							...
							lines.push(
								`§eLitematicDebug | §fStatus > §i导入 §f${j.fileName}\n` +
						...
						if (this.verifyJob) {
							...
							lines.push(
								`§eLitematicDebug | §fStatus > §i世界检查 §f(任务 #${v.taskId}: ${v.fileName})\n` +
						...
						if (this.fixJob) {
							...
							lines.push(
								`§eLitematicDebug | §fStatus > §i修复 §f(任务 #${f.taskId}: ${f.fileName})\n` +
								`§f进度: ${f.done} / ${f.total} 方块 | ${elapsed}s`
							);
						}
						if (!lines.length) {
							this.client.tell("§cLitematicDebug | §fError > §i当前没有进行中的任务", sender);
							return;
						}
				...
				Command.create("ld:verify", "检查投影与世界的差异 / 方块映射错误")
					.addString("ID", false)
					.addOptionalString("模式")
					.setFunc(async (sender, id, mode) => {
						const tid = Number(id);
						if (!Litematic.tasks.has(tid)) {
							this.client.tell("§cLitematicDebug | §fError > §i没有找到任务 ID，请先 !ld:create 获取任务 ID", sender);
							return;
						}
						if (mode === "map") return this.verify(tid, sender);
						if (mode !== undefined && mode !== "world") {
							this.client.tell("§cLitematicDebug | §fError > §i模式参数无效：应为 map（检查方块映射）或留空（检查世界一致性）", sender);
							return;
						}
				...
				Command.create("ld:fix", "修复被挖掉的方块 / 替换无法映射的方块")
					.addString("ID", false)
					...
					.setFunc((sender, id, fb) => {
						...
						if (this.fixJob) {
							this.client.tell("§cLitematicDebug | §fError > §i已有修复任务进行中，请等待完成或 !ld:n 中断", sender);
							return;
						}
	...
	pageList(sender, files, header) {
		...
		if (!files.length) {
			this.client.tell("§cLitematicDebug | §fError > §i没有找到 .litematic 文件", sender);
			return;
		}
	...
	listTasks(sender) {
		...
		if (!tasks.length) {
			this.client.tell("§cLitematicDebug | §fError > §i当前没有任务，先 !ld:create 创建", sender);
			return;
		}
		const now = Date.now();
		...
		this.client.tell(
			`§eLitematicDebug | §fID > §i任务列表 (${tasks.length} 个)\n${lines}\n` +
			`§7使用 §a!ld:verify <ID>§7 检查世界差异 / §a!ld:fix <ID>§7 修复`, sender
		);
	}
	...
	async create(fileName, sender, x, y, z, mode) {
		...
		if (raw === null) {
			this.client.tell("§cLitematicDebug | §fError > §i模式参数无效：应为 raw（保留原始高度）或 trim（裁剪底部空气，默认）", sender);
			return;
		}
		...
		if (typeof fileName !== "string" || !fileName ||
			...
			this.client.tell(`§cLitematicDebug | §fError > §i非法的文件名: ${fileName}`, sender);
			return;
		}
		...
		try {
			...
			this.client.tell(`§cLitematicDebug | §fError > §i解析失败: ${e.message}`, sender);
			return;
		}
		if (!raw) trimAir(data);
		...
		if (coords.length === 3) {
			...
			try {
				...
				if (!pos) {
					this.client.tell("§cLitematicDebug | §fError > §i无法获取你的坐标", sender);
					return;
				}
		...
		if (origin.y < -64 || origin.y + data.sy - 1 > 320) {
			this.client.tell(`§cLitematicDebug | §fError > §iY 轴超出限制: ${origin.y} ~ ${origin.y + data.sy - 1} (允许 -64 ~ 320)`, sender);
			return;
		}
		...
		this.client.tellAll(
			`§eLitematicDebug | §fImport > §i${fileName}\n` +
			`§f任务ID: §b${taskId} §7(用于 !ld:verify / !ld:fix)\n` +
		...
		if (unmappedCount) {
			this.client.tellAll(`§cLitematicDebug | §fWarn > §i无法映射方块: ${unmappedCount} 个 （可用 !ld:verify ${taskId} map 检查，!ld:fix ${taskId} 修复）`);
		}
		this.client.tellAll(`§f确认请发送 §e!ld:y，取消请发送 §c!ld:n`);
	}
	...
	async run() {
		...
		console.log(`[DEBUG] 开始导入: ${file}, 方块: ${total}, 指令: ${totalCmds}, 区域: ${areas.length}, 范围Y: ${origin.y} ~ ${origin.y + sy - 1}`);
		...
		for (let i = 0; i < areas.length; i++) {
			...
			try {
				const r1 = await this.client.runCommand(`/tickingarea add ${absX1} ${origin.y} ${absZ1} ${absX2} ${origin.y + sy - 1} ${absZ2} litematic_${i}`);
				if (r1 && r1.body && r1.body.statusCode !== 0) {
					console.log(`[LitematicDebug] [tickingarea add] ${r1.body.statusMessage}`);
					this.client.tellAll(`§cLitematicDebug | §fError > §i[tickingarea add] ${r1.body.statusMessage}`);
				}
			} catch (e) {
				console.log(`[LitematicDebug] [tickingarea add] 异常: ${e.message}`);
				this.client.tellAll(`§cLitematicDebug | §fError > §i[tickingarea add] 异常: ${e.message}`);
			}
			...
			if (maxYLayersPerChunk >= sy) {
				const f1 = await this.client.runCommand(`/fill ${fillX1} ${origin.y} ${fillZ1} ${fillX2} ${origin.y + sy - 1} ${fillZ2} air`);
				if (f1 && f1.body && f1.body.statusCode !== 0) {
					console.log(`[LitematicDebug] [fill air] ${f1.body.statusMessage}`);
					this.client.tellAll(`§cLitematicDebug | §fError > §i[fill air] ${f1.body.statusMessage}`);
				}
				...
				for (let yStart = 0; yStart < sy; yStart += maxYLayersPerChunk) {
					...
					const f2 = await this.client.runCommand(`/fill ${fillX1} ${absY1} ${fillZ1} ${fillX2} ${absY2} ${fillZ2} air`);
					if (f2 && f2.body && f2.body.statusCode !== 0) {
						console.log(`[LitematicDebug] [fill air] ${f2.body.statusMessage}`);
						this.client.tellAll(`§cLitematicDebug | §fError > §i[fill air] ${f2.body.statusMessage}`);
					}
			...
			for (const cr of chunkRects) {
				...
				try {
					...
					if (result && result.body && result.body.statusCode !== 0) {
						console.log(`[${r.type}] (${cx1},${cy1},${cz1}) ${r.cmd} => ${result.body.statusMessage}`);
						errorCount++;
					} else {
						successCount++;
					}
				} catch (e) {
					console.log(`[${r.type}] (${cx1},${cy1},${cz1}) ${r.cmd} => 异常: ${e.message}`);
					errorCount++;
				}
			...
			this.job.phaseBlockTotal = 0;
			try {
				const r2 = await this.client.runCommand(`/tickingarea remove litematic_${i}`);
				if (r2 && r2.body && r2.body.statusCode !== 0) {
					console.log(`[LitematicDebug] [tickingarea remove] ${r2.body.statusMessage}`);
					this.client.tellAll(`§cLitematicDebug | §fError > §i[tickingarea remove] ${r2.body.statusMessage}`);
				}
			} catch (e) {
				console.log(`[LitematicDebug] [tickingarea remove] 异常: ${e.message}`);
				this.client.tellAll(`§cLitematicDebug | §fError > §i[tickingarea remove] 异常: ${e.message}`);
			}
		}
		...
		if (!this.job.cancelled) {
			...
			this.client.tellAll(`§eLitematicDebug | §fImport > §i${file} 导入完成 共 ${total} 方块 ${totalCmds} 指令 耗时 ${elapsed}s 速度 ${speed}方块/s` +
				(errorCount > 0 ? `\n§c失败 ${errorCount} 条指令` : ""));
		} else {
			this.client.tellAll(`§cLitematicDebug | §fCancel > §i导入已中断 (${file})`);
		}
		this.job = null;
	}
	...
	verify(id, sender) {
		...
		if (!unmapped.length) {
			this.client.tell(
				`§aLitematicDebug | §fVerify > §i任务 #${id} (${task.file}) 方块映射检查通过，无 mod 方块错误\n` +
				`§f提示: 要检查游戏世界里方块是否被挖掉/替换，请用 !ld:verify ${id}`, sender
			);
			return;
		}
		...
		this.client.tell(
			`§eLitematicDebug | §fVerify > §i方块检查报告 (任务 #${id})\n` +
			`§f文件: ${task.file}\n` +
			`§c无法映射方块: ${unmapped.length} 个 （导入时会被跳过）\n` +
			`${lines}\n` +
			`§7发送 !ld:fix ${id} §7将用 stone 替换（可指定替代方块）`, sender
		);
	}
	...
	async verifyWorld(id, sender) {
		...
		const blocks = data.blocks;
		if (!blocks.length) {
			this.client.tell(`§aLitematicDebug | §fVerify > §i任务 #${id} 没有可检查的方块`, sender);
			return;
		}
		const t0 = Date.now();
		const est = Math.ceil(blocks.length / 8);
		this.client.tell(`§i开始世界检查… ${blocks.length} 个方块 （预计 ${est}s 左右，!ld:n 可中断）`, sender);
		const CONC = 4;
		const mismatches = [];
		let checked = 0;
		const delay = ms => new Promise(r => setTimeout(r, ms));
		...
		try {
			...
			for (let i = 0; i < blocks.length && !this.verifyJob.cancelled; i += CONC) {
				...
				if (checked >= 500 && checked % 500 === 0) {
					this.client.tellAll(`§7LitematicDebug | §fVerify >  §i世界检查进度: ${checked}/${blocks.length} | 不匹配: ${mismatches.length} 个`);
				}
				await delay(1);
			}
		} finally {
		...
		if (checked === 0) {
			this.client.tell("§cLitematicDebug | §fVerify > §i世界检查已中断", sender);
			return;
		}
		...
		if (!mismatches.length) {
			this.client.tell(`§aLitematicDebug | §fVerify > §i世界检查完成 (任务 #${id}) 已逐块 testforblock 比对 ${checked} 个方块，全部与投影一致 耗时 ${elapsed}s`, sender);
			return;
		}
		...
		this.client.tell(
			`§eLitematicDebug | §fVerify > §i世界检查报告 (任务 #${id})\n` +
			`§f文件: ${task.file}\n` +
			...
			`§7发送 !ld:fix ${id} §7可重新放置这些方块，恢复与投影一致`, sender
		);
	}
	...
	async fix(id, sender, fb) {
		...
		if (!n1 && !n2) {
			this.client.tell(`§aLitematicDebug | §fFix > §i任务 #${id} 没有需要修复的方块`, sender);
			return;
		}
		const lines = [];
		if (n1) {
			lines.push(`§a已重新放置 ${fixed.length} / ${n1} 个方块 （恢复与投影一致）`);
			if (failed.length) lines.push(`§c未成功 ${failed.length} 个 （命令被服务器限流，可再次 !ld:verify ${id} 检查并 !ld:fix ${id} 重试）`);
		}
		if (n2) lines.push(`§a已将 ${n2} 个无法映射方块替换为 ${fb || "minecraft:stone"}，任务存档已更新，直接发送 !ld:y 即可用修复后的数据导入`);
		if (fixed.length) lines.push(fixed.slice(0, 20).join("\n"));
		this.client.tell(`§aLitematicDebug | §fFix > §i已修复 (任务 #${id})\n${lines.join("\n")}`, sender);
	}
	...
	async preview(fileName, sender, x, y, z, mode) {
		...
		this.client.tell(
			`§eLitematicDebug | §fPreview > §i已生成预览: ${fileName} 尺寸 ${data.sx}×${data.sy}×${data.sz}\n` +
			...
			`§f§o实体标记持续显示，输入 !ld:unpreview 清除`, sender
		);
	}
	...
	clearPreview(sender) {
		...
		if (sender) this.client.tell("§7LitematicDebug | §fPreview > §i已清除建筑预览", sender);
	}
	...
	async exportStructure(fileName, sender, exportName, mode) {
		...
		if (mode === "raw") raw = true;
			...
			this.client.tell("§cLitematicDebug | §fError > §i模式参数无效：应为 raw 或 trim", sender);
			return;
		}
		...
		try {
			...
			this.client.tell(`§cLitematicDebug | §fError > §i导出失败: ${e.message}`, sender);
			return;
		}
		this.client.tell(
			`§aLitematicDebug | §fExport > §i已导出结构文件: ${outPath}\n` +
